...
const DataTable = ({ data, setData, setEditingData }: any) => {
  const handleDelete = async (id: any) => {
    try {
      ...
      if (response.status === 200) {
        setData(data.filter((row: any) => row.data_id !== id));
  ...
  return (
    <div className="mt-10">
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Stored Data</h2>
        ...
        <table className="min-w-full bg-gray-700">
          ...
          <tbody>
            {data.map((row: any, index: number) => (
              <tr key={row.data_id} className={index % 2 === 0 ? 'bg-gray-300' : 'bg-gray-400'}>
